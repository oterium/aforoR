...
| **Curvature Scale Space (CSS)** | Multi-scale Geometry | Contour Gaussian smoothing | Translation, Rotation, Uniform Scale | `aforoR` | Explicit morphological feature detection (lobes, indentations), partial matching. |
...
> **Note:** Even when shapes are mathematically scaled to match the consensus during GPA, `process_images()` safely computes fundamental physical morphometrics (Area, Perimeter, Length in mm) from the *original raw contours* to prevent the loss of real-world biological size metrics.
