...
```{r zonal_plot, echo=FALSE, fig.cap="Zonal Analysis showing contour and sectors", out.width="80%"}
zonal_img <- file.path(work_dir, "Polar", "zonal otolith.jpg")
if (file.exists(zonal_img)) {
  knitr::include_graphics(zonal_img)
} else {
  message("Zonal image not found at: ", zonal_img)
}
```
...
```{r wavelet_plot, echo=FALSE, fig.cap="Wavelet Transform (Scale 5)", out.width="80%"}
wavelet_img <- file.path(work_dir, "Polar", "wavelet otolith.jpg")
if (file.exists(wavelet_img)) {
  knitr::include_graphics(wavelet_img)
} else {
  message("Wavelet image not found at: ", wavelet_img)
}
```
