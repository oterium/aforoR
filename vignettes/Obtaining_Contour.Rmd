...
author: "Jośe Luís Otero-Ferrer & Amalia Manjabacas"
date: "`r Sys.Date()`"
vignette: >
...
The package works by processing all `.jpg` images within a specified folder. For this tutorial, we will use an example image included in the package.
...
The function creates two subdirectories: `Procesamiento` (Polar coordinates) and `Procesamiento2` (Perimeter coordinates).
...
```{r morpho_results}
morpho_file <- file.path(work_dir, "Procesamiento", "MorphometricsEN.csv")
...
```{r wavelet_results}
wavelet_file <- file.path(work_dir, "Procesamiento", "Wavelet_5EN.csv")
...
The package also generates images to verify the contour extraction and analysis zones.

*   `zonal [ImageName].jpg`: Shows the contour and valid zones.
*   `wavelet [ImageName].jpg`: Shows the wavelet transform.

(Images are saved in the `Procesamiento` folder).
