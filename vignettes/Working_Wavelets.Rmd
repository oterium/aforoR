...
# This dataset contains wavelet coefficients at scale 5
data(Aphanopus_W5)

# For comparison with the manual protocol, we split it by species
Acar <- Aphanopus_W5 %>% filter(Species == "Acar")
Aint <- Aphanopus_W5 %>% filter(Species == "Aint")

# Step 2: Check structure
str(Acar) # W1..W512 are numeric variables
str(Aint)
...
meanAphanopusM<-aggregate(Aphanopus_full[, 3:514], list(Aphanopus$Species), mean)
meanAphanopusSD<-aggregate(Aphanopus_full[, 3:514], list(Aphanopus$Species), sd)
